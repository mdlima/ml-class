...
for i=1:m
  ...
  [theta, J, exit_flag] = fminunc(@(t)(costFunctionReg(t, Xtrain(1:i, :), ytrain(1:i), lambda)), initial_theta, options);
  
  % Compute error for i elements training
  [cost, grad] = costFunctionReg(theta, Xtrain(1:i, :), ytrain(1:i), 0);
  error_train(i) = cost;
  
  % Compute error for cross-validation test
  [cost, grad] = costFunctionReg(theta, Xval, yval, 0);
  error_val(i) = cost;
